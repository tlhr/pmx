...
"""
    ...
    parser.add_argument(
        ...
        dest="pro_top",
        type=str,
        help="Input topology file for the protein. " 'Default is "protein.top".',
        default="protein.top",
    )
    parser.add_argument(
        "-lt",
...
def main(args):
    ...
    # ------------------------------------------
    restraints = AbsRestraints(
        protein=pro,
        ligand=lig,
        pro_ids=args.pro_ids,
        lig_ids=args.lig_ids,
        seed=args.seed,
    )
    ...
    if args.build is False:
        ...
        comtop.ii = restraints.make_ii(
            switch_on=args.restr_switch_on, ligand_first=True
        )
    ...
    if args.build is True:
        ...
        if args.doublebox is True and goodtogo is True:
            ...
            mout = double_box(
                ...
                r=2.5,
                d=1.5,
                bLongestAxis=args.longest_axis,
                verbose=False,
            )
            ...
            doubletop.include_itps = [
                ("ligandAB.itp", "top"),
                ("ligandBA.itp", "bottom"),
            ]
            doubletop.molecules.append([ligtopBA.name, 1])
            # merge atomtypes (basically add the dummies)
            doubletop.atomtypes = merge_atomtypes(
                doubletop.atomtypes, ligtopAB.atomtypes
            )
            ...
            # -----------------
            gmx.solvate(
                cp="doublebox.gro", cs="spc216.gro", p="doublebox.top", o="solvate.gro"
            )
            ...
            doubletop.ii = restraints.make_ii(
                switch_on=args.restr_switch_on, ligand_first=True
            )
            doubletop.write("doublebox.top", stateBonded="A")
        ...
        # ==================================
            ...
            gmx.solvate(
                cp="editconf.gro", cs="spc216.gro", p="complex.top", o="solvate.gro"
            )
            gmx.write_mdp(mdp="enmin", fout="genion.mdp")
            gmx.grompp(
                f="genion.mdp",
            ...
            gmx.genion(
                s="genion.tpr", p="complex.top", o="genion.gro", conc=0.15, neutral=True
            )
            ...
            ligtop.footer = [
                '#include "{ff}.ff/tip3p.itp"'.format(ff=ligtop.forcefield),
                "#ifdef POSRES_WATER",
                "[ position_restraints ]",
                "1    1       1000       1000       1000",
                "#endif",
            ...
            ligtop.write(
                "ligand.top", stateBonded="A", write_atypes=True, posre_include=True
            )

            # run gromacs setup
            gmx.editconf(f="ligand.gro", o="editconf.gro", bt="cubic", d=1.2)
            gmx.solvate(
                cp="editconf.gro", cs="spc216.gro", p="ligand.top", o="solvate.gro"
            )
            gmx.write_mdp(mdp="enmin", fout="genion.mdp")
            gmx.grompp(
                f="genion.mdp",
            ...
            gmx.genion(
                s="genion.tpr", p="ligand.top", o="genion.gro", conc=0.15, neutral=True
            )
