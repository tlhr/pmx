...
if __name__== "__main__":
    ...
    for i in range(3):
        ...
        if(args.reuse_existing_pbc_fixes and os.path.exists("trj_{}.trr".format(names[i]))):
            continue;
        elif(args.reuse_existing_pbc_fixes and args.verbose):
    ...
    #Use mdanalysis for alignment. Should be fater than pmx.
    u_A = md.Universe(args.fH+"/confout.gro", "trj_A.trr")
    u_B = md.Universe(args.fA+"/confout.gro", "trj_B.trr")
    u_C = md.Universe(args.fW+"/confout.gro", "trj_C.trr")
